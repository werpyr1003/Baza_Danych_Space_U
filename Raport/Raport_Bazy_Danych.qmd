...
Firma Space-U jest frimą turystyczną organizującą wyprawy kosmiczne,
...
`r planetary_system_count` systemach planetranych
...
analysis_data %>% select(trip_name, bookings_count, total_costs, total_revenue, profit, profit_margin)
...
`r formatC(analysis_data$profit[1] / 1e6, format = "f", digits = 2)` mln 
zysku brutto\
...
najlepiej radzą sobie misje związane z:
`r paste(analysis_data %>% mutate(avg_profit = profit / bookings_count) %>% arrange(desc(avg_profit)) %>% head(3) %>% pull(trip_name), collapse = ", ")`.
Generują one największy zwrot z pojedynczej podróży, często kilkakrotnie
wyższy niż wyprawy mniej opłacalne.
...
# Porównanie wycieczek w jedną stronę z wycieczkami "tam i spowrotem"
...
ggplot(planets_ratio, aes(x = reorder(planet, round_trip_ratio), y = round_trip_ratio)) +
...
`r top_planets$planet`, gdzie współczynnik wyniósł
...
Najwyższą medianę współczynnika round-trip zaobserwowano dla cechy
`r paste(   df_wide %>%     group_by(special_features) %>%     summarise(med = median(round_trip_ratio, na.rm = TRUE)) %>%     filter(med == max(med, na.rm = TRUE)) %>%     pull(special_features),   collapse = ", " )`
...
m.in. `r paste(head(top_planets$planet, 5), collapse = ", ")`, z
maksymalnym wskaźnikiem `r round(top_planets$round_trip_ratio[1], 2)`.
Cechy specjalne mają wpływ na preferencje — najwyższą medianę round-trip
odnotowano dla planet z cechą
`r df_wide %>% group_by(special_features) %>% summarise(med = median(round_trip_ratio, na.rm = TRUE)) %>% arrange(desc(med)) %>% slice(1) %>% pull(special_features)`.
Zauważono też
`r ifelse(cor(df_wide$orbit_period_days, df_wide$round_trip_ratio, use = "complete.obs") > 0, "dodatnią", "ujemną")`
korelację między okresem orbitalnym a preferencją round-trip (r =
`r round(cor(df_wide$orbit_period_days, df_wide$round_trip_ratio, use = "complete.obs"), 2)`).
...
Analziując wykres pokazujący zarobki pracowników, w top 3 najlepiej
zarabiającmi pracownikami są `r top_3_employees_names`, którzy zarabiają
między `r min_salary_rounded` a `r max_salary_rounded` USD. Zamują oni stanowiska `r top3_role_in_trip`
...
Tabela pokazuje 10 układów planetarnych generujących największy zysk
netto, co pozwala wskazać najbardziej dochodowe kierunki wypraw.
...
generująca łącznie
`r scales::dollar(galaxy_profit %>% summarise(max(total_net_profit)) %>% pull())`.
Najmniej zyskowną galaktyką jest
`r galaxy_profit %>% slice_min(total_net_profit) %>% pull(galaxy_name)`
z łącznym zyskiem netto
`r scales::dollar(galaxy_profit %>% summarise(min(total_net_profit)) %>% pull())`.
...
`r scales::dollar(avg_profit_per_trip %>% summarise(max(avg_profit)) %>% pull())`
...
`r monthly_summary$month[which.max(monthly_summary$total_trips)]`,
osiągając `r max(monthly_summary$total_trips)` wypraw. Z kolei
najmniejsza liczba wypraw przypada na miesiąc
`r monthly_summary$month[which.min(monthly_summary$total_trips)]`, z
liczbą `r min(monthly_summary$total_trips)`. Różnice sezonowe są
widoczne.
...
Najwyższy miesięczny przychód został osiągnięty w miesiącu
`r monthly_summary$month[which.max(monthly_summary$total_revenue)]`,
wynosząc
`r scales::dollar(max(monthly_summary$total_revenue, na.rm = TRUE))`. Z
kolei najniższy przychód przypadł na miesiąc
`r monthly_summary$month[which.min(monthly_summary$total_revenue)]`,
osiągając
`r scales::dollar(min(monthly_summary$total_revenue, na.rm = TRUE))`.
Tego typu wahania mogą wskazywać na sezonowość przychodów.
```{r, echo = FALSE}
...
Najwięcej zakończonych wypraw odnotowano w miesiącu
`r monthly_status %>% filter(status == "completed") %>% slice_max(count) %>% pull(month)`,
osiągając liczbę
`r monthly_status %>% filter(status == "completed") %>% summarise(max(count)) %>% pull()`.Największa
liczba opóźnionych wypraw wystąpiła w miesiącu
`r monthly_status %>% filter(status == "delayed") %>% slice_max(count) %>% pull(month)`,
a liczba tych wypraw wyniosła
`r monthly_status %>% filter(status == "delayed") %>% summarise(max(count)) %>% pull()`.W
przypadku anulowanych wypraw szczyt przypadł na miesiąc
`r monthly_status %>% filter(status == "cancelled") %>% slice_max(count) %>% pull(month)`,
gdzie naliczono
`r monthly_status %>% filter(status == "cancelled") %>% summarise(max(count)) %>% pull()`
przypadków.
...
generując
`r scales::dollar(quarterly_summary %>% summarise(max(total_revenue)) %>% pull())`.
Natomiast najniższy przychód zanotowano w kwartale
`r quarterly_summary %>% slice_min(total_revenue) %>% pull(quarter)`, z
wartością
`r scales::dollar(quarterly_summary %>% summarise(min(total_revenue)) %>% pull())`.
